...
def test_complex_dense_network():
    ...
    flights = [
        ...
        Flight(0, 0, 0, 1, 120, 300),      # NY -> Chicago: 0-120
        Flight(1, 0, 60, 1, 180, 250),     # NY -> Chicago: 60-180
        Flight(2, 0, 0, 3, 360, 800),      # NY -> LA: 0-360 (direct but long)
        Flight(3, 0, 0, 2, 180, 500),      # NY -> Denver: 0-180
        
        # Connections through Chicago (1)
        Flight(4, 1, 140, 3, 300, 200),    # Chicago -> LA: 140-300 (faster path)
        Flight(5, 1, 200, 3, 360, 180),    # Chicago -> LA: 200-360
        
        # Connections through Denver (2)
        Flight(6, 2, 200, 3, 380, 200),    # Denver -> LA: 200-380
        
        # Some return flights
        Flight(7, 1, 400, 0, 520, 300),    # Chicago -> NY: 400-520
        Flight(8, 3, 400, 0, 760, 800),    # LA -> NY: 400-760
        
        # Late flights
        Flight(9, 0, 500, 3, 860, 700),   # NY -> LA: 500-860 (late direct)

        Flight(10, 2, 200, 3, 290, 100)    # Denver -> LA: 200-290
    ]
    ...
    test_scenarios = [
        # Basic direct flight test
        {
            ...
            "expected_flights": 1,
            "expected_flight_numbers": [0],  # Should take first direct flight
        ...
        # Multi-hop vs Direct comparison
        {
            ...
            "expected_flights": 1,
            "expected_flight_numbers": [2],  # Should take direct flight even if longer
        ...
        # Time window forcing specific route
        {
            ...
            "expected_flights": 2,
            "expected_flight_numbers": [3, 10],  # Must go through Denver due to time constraint
        ...
        # Testing buffer time constraints
        {
            "start": 0, "end": 3, "t1": 0, "t2": 1000,
            "expected_flights": 1,
            "expected_flight_numbers": [2],  # Direct flight is better than tight connections
